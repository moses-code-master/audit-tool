...
$$('.mode-tab').forEach((tab) => {
  tab.addEventListener('click', () => {
    ...
    $('.paste-field').classList.toggle('hidden', tab.dataset.mode !== 'paste');
    const labels = {
    ...
    $('#urlLabel').textContent = labels[tab.dataset.mode][0];
    $('#articleUrl').placeholder = labels[tab.dataset.mode][1];
  });
});
...
function renderDashboard(data) {
  ...
  el.scoreArc.style.strokeDashoffset = `${314 - (314 * score) / 100}`;
  el.title.textContent = article.title || 'Untitled article';
  el.link.textContent = article.url || '';
  el.link.href = String(article.url || '').startsWith('http') ? article.url : '#';
  el.heroSummary.textContent = article.metaDescription || article.bodyPreview || 'No summary was extracted.';
  el.categoryTag.textContent = article.category || 'News article';
  el.auditTimestamp.textContent = `Audited ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
  el.author.textContent = article.author || 'Not found';
  el.date.textContent = article.date || 'Not found';
  el.wordCount.textContent = Number(article.wordCount || 0).toLocaleString();
  el.readTime.textContent = `${readTime} min`;
  el.heroConfidence.innerHTML = pill(confidence.label, confidence.status);
  el.heroAiRisk.innerHTML = pill(aiRisk.label, aiRisk.status);
  el.auditRuntime.textContent = `Audit completed in ${data.auditDuration || '-'}s`;

  renderKpis(data, confidence, aiRisk);
  renderScoreAnalytics(data);
  renderQuality(data);
  renderContent(data);
  renderIssues(data);
  renderIntel(data);
...
function renderKpis(data, confidence, aiRisk) {
  ...
  el.kpiGrid.innerHTML = kpis.map(([icon, label, value, status, detail]) => `<article class='kpi-card ${status}'><span class='kpi-icon'>${esc(icon)}</span><div><p>${esc(label)}</p><strong>${esc(value)}</strong><small>${esc(detail)}</small></div>${pill(labelFor(status), status)}</article>`).join('');
}

function renderScoreAnalytics(data) {
  const entries = Object.entries(data.scores || {});
  el.scoreAnalytics.innerHTML = `<div class='panel-heading'><div><p class='eyebrow'>Editorial scoring</p><h3>Score Breakdown</h3></div></div><div class='score-bars'>${entries.map(([name, score]) => `<div class='score-bar-row'><span>${esc(name)}</span><div class='bar-track'><i class='bar-fill' style='width:${Number(score || 0)}%'></i></div><strong class='score-value'>${esc(score)}/100</strong></div>`).join('')}</div>`;
}

function renderQuality(data) {
  const gates = data.gates || [];
  const totals = countStatuses(gates);
  el.qualityPanel.innerHTML = `<div class='panel-heading'><div><p class='eyebrow'>Pass, warning, fail</p><h3>Quality Gates</h3></div></div><div class='gate-summary'><strong class='pass'>${totals.pass || 0}<span>Pass</span></strong><strong class='warning'>${totals.warning || 0}<span>Warning</span></strong><strong class='fail'>${totals.fail || 0}<span>Fail</span></strong></div><div class='gate-list'>${gates.sort((a, b) => rank(a.status) - rank(b.status)).slice(0, 8).map((gate) => `<details class='gate-line ${gate.status}' ${gate.status !== 'pass' ? 'open' : ''}><summary><span>${gate.status === 'pass' ? 'OK' : gate.status === 'warning' ? '!' : 'X'}</span>${esc(gate.name)}${pill(gate.status, gate.status)}</summary><p>${esc(gate.detail)}</p></details>`).join('')}</div>`;
}
...
function renderContent(data) {
  ...
  el.contentGlance.innerHTML = `<div class='panel-heading'><div><p class='eyebrow'>Content At-A-Glance</p><h3>Extracted signals</h3></div></div><div class='glance-grid'>${cards.map(([label, value]) => `<article><span>${esc(label)}</span><strong>${esc(value)}</strong></article>`).join('')}</div>`;
  el.outlinePanel.innerHTML = `<div class='panel-heading'><div><p class='eyebrow'>Structure</p><h3>Top H2 Headings</h3></div></div><ol class='outline-list'>${(article.h2Headings?.length ? article.h2Headings : ['No H2 headings found']).slice(0, 5).map((heading) => `<li>${esc(heading)}</li>`).join('')}</ol>`;
...
function renderRisk(data) {
  ...
  el.risk.innerHTML = `<div class='panel-heading'><div><p class='eyebrow'>Publishing Risk</p><h3>Risk register</h3></div></div><div class='risk-list'>${risks.map(([name, level]) => `<article><span>${esc(name)}</span>${pill(level, level === 'High' ? 'fail' : level === 'Medium' ? 'warning' : 'pass')}</article>`).join('')}</div>`;
}
...
function renderDiagnostics(data) {
  ...
  el.diagnostics.innerHTML = `<div class='panel-heading'><div><p class='eyebrow'>Readability</p><h3>Formula diagnostics</h3></div></div><div class='diagnostics-grid'>${items.map(([label, value]) => `<article><span>${esc(label)}</span><strong>${esc(value)}</strong></article>`).join('')}</div>`;
}
